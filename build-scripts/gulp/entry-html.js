...
gulp.task("gen-index-oppio-dev", async () => {
  writeOppioEntrypoint(
    `${paths.oppio_publicPath}/frontend_latest/entrypoint.js`,
    `${paths.oppio_publicPath}/frontend_es5/entrypoint.js`
  );
});

gulp.task("gen-index-oppio-prod", async () => {
  const latestManifest = require(path.resolve(
    paths.oppio_output_latest,
    "manifest.json"
  ));
  const es5Manifest = require(path.resolve(
    paths.oppio_output_es5,
    "manifest.json"
  ));
  writeOppioEntrypoint(
...
function writeOppioEntrypoint(latestEntrypoint, es5Entrypoint) {
  fs.mkdirSync(paths.oppio_output_root, { recursive: true });
  // Safari 12 and below does not have a compliant ES2015 implementation of template literals, so we ship ES5
  fs.writeFileSync(
    path.resolve(paths.oppio_output_root, "entrypoint.js"),
    `
function loadES5() {
