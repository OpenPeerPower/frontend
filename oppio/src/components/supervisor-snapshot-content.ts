import { mdiFolder, mdiOpenPeerPower, mdiPuzzle } from "@mdi/js";
...
export class SupervisorSnapshotContent extends LitElement {
  ...
  @property({ attribute: false }) public snapshot?: OppioSnapshotDetail;

  @property() public snapshotType: OppioSnapshotDetail["type"] = "full";
    ...
    if (!this.hasUpdated) {
      this.folders = _computeFolders(
        this.snapshot
          ...
          : ["openpeerpower", "ssl", "share", "media", "addons/local"]
      );
    ...
    return html`
      ${this.snapshot
        ? html`<div class="details">
            ...
            ${this.opp
              ? formatDateTime(new Date(this.snapshot.date), this.opp.locale)
              : this.snapshot.date}
          </div>`
        : html`<paper-input
            name="snapshotName"
              ...
              ? html`
                  <ha-formfield
                    .label=${html`<supervisor-formfield-label
                      ...
                      .version=${this.snapshot.openpeerpower}
                    >
  ...
  public snapshotDetails():
    ...
    if (!this.snapshot) {
      data.name = this.snapshotName || formatDate(new Date(), this.opp.locale);
    }
    ...
    if (this.homeAssistant) {
      data.openpeerpower = this.homeAssistant;
    }
    ...
    this[section].forEach((item) => {
      templates.push(html`<ha-formfield
        .label=${html`<supervisor-formfield-label
          ...
          !this.onboarding &&
          atLeastVersion(this.opp.config.version, 0, 105) &&
          addons?.get(item.slug)?.icon
            ? `/api/oppio/addons/${item.slug}/icon`
            : undefined}
