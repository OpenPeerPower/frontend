...
import {
  ...
  fetchOppioDockerRegistries,
  removeOppioDockerRegistry,
} from "../../../../src/data/oppio/docker";
...
@customElement("dialog-oppio-registries")
class OppioRegistriesDialog extends LitElement {
  ...
  private async _loadRegistries(): Promise<void> {
    const registries = await fetchOppioDockerRegistries(this.opp);
  ...
  private async _addNewRegistry(): Promise<void> {
    ...
    try {
      await addOppioDockerRegistry(this.opp, data);
      await this._loadRegistries();
      this._addingRegistry = false;
    } catch (err) {
  ...
  private async _removeRegistry(ev: Event): Promise<void> {
    ...
    try {
      await removeOppioDockerRegistry(this.opp, entry.registry);
      await this._loadRegistries();
    } catch (err) {
      showAlertDialog(this, {
...
declare global {
  interface HTMLElementTagNameMap {
    "dialog-oppio-registries": OppioRegistriesDialog;
  }
}
