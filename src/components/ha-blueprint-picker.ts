...
@customElement("op-blueprint-picker")
class HaBluePrintPicker extends LitElement {
  ...
  protected render(): TemplateResult {
    ...
    return html`
      <paper-dropdown-menu-light
        .label=${this.label ||
        this.opp.localize("ui.components.blueprint-picker.label")}
  ...
  protected firstUpdated(changedProps) {
    ...
    if (this.blueprints === undefined) {
      fetchBlueprints(this.opp!, this.domain).then((blueprints) => {
...
declare global {
  interface HTMLElementTagNameMap {
    "op-blueprint-picker": HaBluePrintPicker;
  }
}
