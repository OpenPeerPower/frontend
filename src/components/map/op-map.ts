...
export class HaMap extends ReactiveElement {
  ...
  protected update(changedProps: PropertyValues) {
    ...
    if (changedProps.has("_loaded") || changedProps.has("entities")) {
      this._drawEntities();
    } else if (this._loaded && oldOpp && this.entities) {
      // Check if any state has changed
      for (const entity of this.entities) {
        if (
          oldOpp.states[getEntityId(entity)] !==
          this.opp!.states[getEntityId(entity)]
        ) {
          this._drawEntities();
          break;
        }
    ...
    if (
      ...
      (!changedProps.has("opp") ||
        (oldOpp && oldOpp.themes.darkMode === this.opp.themes.darkMode))
    ) {
      return;
    }
    const darkMode = this.darkMode ?? this.opp.themes.darkMode;
  ...
  public fitMap(): void {
    if (!this.leafletMap || !this.Leaflet || !this.opp) {
      return;
    }
  ...
  private _drawEntities(): void {
    ...
    const className =
      this.darkMode ?? this.opp.themes.darkMode ? "dark" : "light";

    for (const entity of this.entities) {
      const stateObj = opp.states[getEntityId(entity)];
      if (!stateObj) {
        continue;
      }
