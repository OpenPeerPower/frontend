  ...
  protected updated(changedProps: PropertyValues) {
    if (changedProps.has("opp")) {
      ...
      if (!oldOpp || oldOpp.locale !== this.opp.locale) {
        this._hour24format = !useAmPm(this.opp.locale);
        this._rtlDirection = computeRTLDirection(this.opp);
      }
    }
  }
    ...
    return html`
      ...
      >
        <div slot="input" class="date-range-inputs">
          <op-svg-icon .path=${mdiCalendar}></op-svg-icon>
          <paper-input
            .value=${formatDateTime(this.startDate, this.opp.locale)}
          ...
          ></paper-input>
          <paper-input
            .value=${formatDateTime(this.endDate, this.opp.locale)}
            label=${this.opp.localize(
