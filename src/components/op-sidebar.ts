...
class HaSidebar extends LitElement {
  ...
  protected shouldUpdate(changedProps: PropertyValues): boolean {
    ...
    return (
      ...
      opp.locale !== oldOpp.locale ||
      opp.states !== oldOpp.states ||
      opp.defaultPanel !== oldOpp.defaultPanel
    );
  }
  ...
  private _renderHeader() {
    ...
    >
      ...
      ${this.editMode
        ...
        : html`<div class="title">Open Peer Power</div>`}
    </div>`;
  }
  ...
  private _renderHiddenPanels() {
    return html` ${this._hiddenPanels.length
      ? html`${this._hiddenPanels.map((url) => {
          const panel = this.opp.panels[url];
          if (!panel) {
            return "";
          }
          ...
          >
            ...
            <span class="item-text"
              >${panel.url_path === this.opp.defaultPanel
                ...
                : this.opp.localize(`panel.${panel.title}`) ||
                  panel.title}</span
            >
  ...
  private _renderNotifications() {
    let notificationCount = this._notifications
      ...
      : 0;
    for (const entityId in this.opp.states) {
  ...
  private _renderUserItem() {
    return html`<a
      class=${classMap({
        ...
        "iron-selected": this.opp.panelUrl === "profile",
      })}
      href="/profile"
      data-panel="panel"
      tabindex="-1"
  ...
  private _closeEditMode() {
    fireEvent(this, "opp-edit-sidebar", { editMode: false });
  }
  ...
  private _renderPanels(panels: PanelInfo[]) {
    return panels.map((panel) =>
      this._renderPanel(
        panel.url_path,
        panel.url_path === this.opp.defaultPanel
          ? panel.title || this.opp.localize("panel.states")
          : this.opp.localize(`panel.${panel.title}`) || panel.title,
        panel.icon,
        panel.url_path === this.opp.defaultPanel && !panel.icon
          ? mdiViewDashboard
          : undefined
      )
    );
