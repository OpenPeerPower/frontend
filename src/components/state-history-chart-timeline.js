...
class StateHistoryChartTimeline extends LocalizeMixin(PolymerElement) {
  ...
  drawChart() {
    ...
    stateHistory.forEach((stateInfo) => {
      ...
      const invertOnOff =
        ...
        BINARY_SENSOR_DEVICE_CLASS_COLOR_INVERTED.has(
          this.opp.states[stateInfo.entity_id].attributes.device_class
        );
    ...
    const formatTooltipLabel = (item, data) => {
      ...
      const start = formatDateTimeWithSeconds(values[0], this.opp.locale);
      const end = formatDateTimeWithSeconds(values[1], this.opp.locale);
    ...
    const formatTooltipBeforeBody = (item, data) => {
      if (!this.opp.userData || !this.opp.userData.showAdvanced || !item[0]) {
        return "";
      }
