...
import { CurrentUser, OpenPeerPower } from "../../types";
...
class UserBadge extends LitElement {
  ...
  public willUpdate(changedProps: PropertyValues<this>) {
    ...
    if (
      ...
      oldOpp &&
      this.opp.states[this._personEntityId] !==
        oldOpp.states[this._personEntityId]
    ) {
      const entityState = this.opp.states[this._personEntityId];
      if (entityState) {
        this._personPicture = entityState.attributes.entity_picture;
      } else {
        this._getPersonPicture();
      }
    } else if (!this._personEntityId && oldOpp) {
      this._getPersonPicture();
    }
  }

  protected render(): TemplateResult {
    if (!this.opp || !this.user) {
      return html``;
    }
  ...
  private _getPersonPicture() {
    ...
    for (const entity of Object.values(this.opp.states)) {
      if (
