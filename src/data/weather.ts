...
interface WeatherEntityAttributes extends OppEntityAttributeBase {
  temperature: number;
  humidity?: number;
...
export interface WeatherEntity extends OppEntityBase {
...
export const getWind = (
  ...
  const speedText = `${formatNumber(speed, opp.locale)} ${getWeatherUnit(
    opp!,
    "wind_speed"
  )}`;
  if (bearing !== null) {
...
export const getWeatherUnit = (
  ...
  switch (measure) {
    ...
    case "precipitation_probability":
      return "%";
    default:
      return opp.config.unit_system[measure] || "";
  }
};
...
export const getSecondaryWeatherAttribute = (
  ...
  const extrema = getWeatherExtrema(opp, stateObj);
...
const getWeatherExtrema = (
  ...
  const unit = getWeatherUnit(opp!, "temperature");
