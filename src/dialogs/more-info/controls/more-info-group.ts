...
class MoreInfoGroup extends LitElement {
  ...
  @internalProperty() private _groupDomainStateObj?: OppEntity;
  ...
  protected updated(changedProperties: PropertyValues) {
    if (
      !this.opp ||
      !this.stateObj ||
      (!changedProperties.has("opp") && !changedProperties.has("stateObj"))
    ) {
      return;
    }

    const states = this.stateObj.attributes.entity_id
      .map((entity_id) => this.opp.states[entity_id])
      .filter((state) => state);
  ...
  protected render(): TemplateResult {
    ...
    ${this.stateObj.attributes.entity_id.map((entity_id) => {
      const state = this.opp!.states[entity_id];
      if (!state) {
        return "";
      }
      return html`
