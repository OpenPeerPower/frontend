...
export class DialogTryTts extends LitElement {
    ...
    return html`
      <ha-dialog
        ...
        .heading=${createCloseHeading(
          this.opp,
          this.opp.localize("ui.panel.config.cloud.account.tts.dialog.header")
        )}
      >
        <div>
          <paper-textarea
  ...
  private async _playExample() {
    ...
    if (target === "browser") {
      ...
      this.opp.callService("tts", "cloud_say", {
        entity_id: target,
        message,
      });
    }
  ...
  private async _playBrowser(message: string, audio: HTMLAudioElement) {
    ...
    let url;
    try {
      const result = await convertTextToSpeech(this.opp, {
        platform: "cloud",
        message,
        language,
        options: { gender },
