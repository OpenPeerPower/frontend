...
export class DialogDeviceAutomation extends LitElement {
  ...
  protected updated(changedProps): void {
    ...
    fetchDeviceActions(this.opp, deviceId).then((actions) => {
      this._actions = actions;
    });
    if (script) {
      return;
    }
    fetchDeviceTriggers(this.opp, deviceId).then((triggers) => {
      this._triggers = triggers;
    });
    fetchDeviceConditions(this.opp, deviceId).then((conditions) => {
