...
export class EntityRegistrySettingsHelper extends LitElement {
  ...
  protected firstUpdated(changedProperties: PropertyValues) {
    ...
    this._componentLoaded = isComponentLoaded(this.opp, this.entry.platform);
  }
    ...
    return html`
      <div class="form">
        ...
        ${!this._componentLoaded
          ...
          : this._item === null
          ? this.opp.localize("ui.dialogs.helper_settings.yaml_not_editable")
          : html`
  ...
  private async _getItem() {
    const items = await HELPERS[this.entry.platform].fetch(this.opp!);
  ...
  private async _confirmDeleteItem(): Promise<void> {
    ...
    try {
      if (this._componentLoaded && this._item) {
        await HELPERS[this.entry.platform].delete(this.opp!, this._item.id);
      } else {
        const stateObj = this.opp.states[this.entry.entity_id];
        if (!stateObj?.attributes.restored) {
          return;
        }
        await removeEntityRegistryEntry(this.opp!, this.entry.entity_id);
      }
      fireEvent(this, "close-dialog");
    } finally {
      this._submitting = false;
