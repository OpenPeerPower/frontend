...
import type { HaTabsSubpageDataTable } from "../../../layouts/opp-tabs-subpage-data-table";
...
export class HaConfigEntities extends SubscribeMixin(LitElement) {
  ...
  private _columns = memoize(
    (narrow, _language): DataTableColumnContainer => ({
        ...
        template: narrow
          ? (name, entity: any) =>
              html`
                ...
                <div class="secondary">
                  ${entity.entity_id} |
                  ${this.opp.localize(`component.${entity.platform}.title`) ||
                  entity.platform}
                </div>
              `
          : undefined,
      ...
      platform: {
        ...
        width: "20%",
        template: (platform) =>
          this.opp.localize(`component.${platform}.title`) || platform,
      },
      area: {
        title: this.opp.localize(
  ...
  private _filteredEntitiesAndDomains = memoize(
    (
      ...
      for (const entry of filteredEntities) {
        const entity = this.opp.states[entry.entity_id];
  ...
  protected updated(changedProps): void {
    ...
    let changed = false;
    if (!this.opp || !this._entities) {
      return;
    }
    if (changedProps.has("opp") || changedProps.has("_entities")) {
      const stateEntities: StateEntity[] = [];
      const regEntityIds = new Set(
        this._entities.map((entity) => entity.entity_id)
      );
      for (const entityId of Object.keys(this.opp.states)) {
        ...
        stateEntities.push({
          name: computeStateName(this.opp.states[entityId]),
          entity_id: entityId,
  ...
  private async _enableSelected() {
    showConfirmationDialog(this, {
      ...
      confirmText: this.opp.localize("ui.common.enable"),
      ...
      confirm: async () => {
        ...
        await Promise.all(
          this._selectedEntities.map(async (entity) => {
            const result = await updateEntityRegistryEntry(this.opp, entity, {
              disabled_by: null,
            });
  ...
  private async _loadConfigEntries() {
    this._entries = await getConfigEntries(this.opp);
  }

  private _clearFilter() {
    if (
      this._activeFilters(this._searchParms, this.opp.localize, this._entries)
    ) {
