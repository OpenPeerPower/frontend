...
export class DialogHelperDetail extends LitElement {
    ...
    return html`
      <op-dialog
        ...
        .heading=${this._platform
          ...
          : this.opp.localize("ui.panel.config.helpers.dialog.add_helper")}
      >
        ${this._platform
          ? html`
          ...
          : html`
              ${Object.keys(HELPERS).map((platform: string) => {
                const isLoaded = isComponentLoaded(this.opp, platform);
                return html`
                  <div class="form">
                    <paper-icon-item
                      .disabled=${!isLoaded}
  ...
  private async _createItem(): Promise<void> {
    ...
    this._error = "";
    try {
      await HELPERS[this._platform](this.opp, this._item);
      this.closeDialog();
    } catch (err) {
