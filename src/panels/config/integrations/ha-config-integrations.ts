...
class HaConfigIntegrations extends SubscribeMixin(LitElement) {
    ...
    (
      ...
      configEntriesInProgress = configEntriesInProgress.map(
        (flow: DataEntryFlowProgressExtended) => ({
          ...flow,
          title: localizeConfigFlowTitle(this.opp.localize, flow),
        })
      );
      if (!filter) {
  ...
  private _handleRemoved(ev: OPPDomEvent<ConfigEntryRemovedEvent>) {
    this._configEntries = this._configEntries!.filter(
      (entry) => entry.entry_id !== ev.detail.entryId
    );
  }

  private _handleUpdated(ev: OPPDomEvent<ConfigEntryUpdatedEvent>) {
  ...
  static get styles(): CSSResult[] {
    return [
      ...
      css`
        ...
        .discovered {
          --op-card-border-color: var(--primary-color);
        }
        .discovered .header {
        ...
        .ignored {
          --op-card-border-color: var(--light-theme-disabled-color);
        }
        .ignored img {
          filter: grayscale(1);
        }
