...
const integrationsWithPanel = {
  oppio: {
    buttonLocalizeKey: "ui.panel.config.oppio.button",
...
export class HaIntegrationCard extends LitElement {
    ...
    return html`
      <ha-card outlined class="group ${classMap({ disabled: this.disabled })}">
        ...
        <div class="group-header">
          ...
          />
          <h2>
            ${domainToName(this.opp.localize, this.domain)}
          </h2>
        </div>
        <paper-listbox>
          ${this.items.map(
  ...
  private async _disableIntegration(configEntry: ConfigEntry) {
    ...
    const result = await disableConfigEntry(this.opp, entryId);
  ...
  private async _enableIntegration(configEntry: ConfigEntry) {
    ...
    const result = await enableConfigEntry(this.opp, entryId);
  ...
  private async _removeIntegration(configEntry: ConfigEntry) {
    ...
    const result = await deleteConfigEntry(this.opp, entryId);
  ...
  private async _reloadIntegration(configEntry: ConfigEntry) {
    ...
    const result = await reloadConfigEntry(this.opp, entryId);
  ...
  private async _editEntryName(ev) {
    ...
    const newEntry = await updateConfigEntry(this.opp, configEntry.entry_id, {
      title: newName,
    });
  ...
  static get styles(): CSSResult[] {
    return [
      ...
      css`
        ...
        .disabled {
          --op-card-border-color: var(--warning-color);
        }
        .not-loaded {
          --op-card-border-color: var(--error-color);
        }
        .header {
          padding: 8px;
          text-align: center;
