...
class OZWNetworkNodes extends LitElement {
  ...
  private _columns = memoizeOne((narrow: boolean): DataTableColumnContainer => {
    return {
      ...
      node_query_stage: {
        title: this.opp.localize("ui.panel.config.ozw.nodes_table.query_stage"),
        sortable: true,
        width: narrow ? "25%" : "15%",
      },
      is_zwave_plus: {
        title: this.opp.localize("ui.panel.config.ozw.nodes_table.zwave_plus"),
        hidden: narrow,
        template: (value: boolean) =>
          value ? html` <op-svg-icon .path=${mdiCheck}></op-svg-icon>` : "",
      },
      is_failed: {
        title: this.opp.localize("ui.panel.config.ozw.nodes_table.failed"),
        hidden: narrow,
        template: (value: boolean) =>
          value ? html` <op-svg-icon .path=${mdiAlert}></op-svg-icon>` : "",
      },
    };
  });
