...
export class ZwaveMigration extends LitElement {
  ...
  private async _getMigrationConfig(): Promise<void> {
    this._migrationConfig = await fetchMigrationConfig(this.opp!);
  }
  ...
  private async _stopNetwork(): Promise<void> {
    ...
    this._unsub = this.opp!.connection.subscribeEvents(
      () => this._networkStopped(),
      "zwave.network_stop"
    );
    this.opp!.callService("zwave", "stop_network");
  }

  private async _setupOzw() {
    const ozwConfigFlow = await startOzwConfigFlow(this.opp);
    if (isComponentLoaded(this.opp, "ozw")) {
    ...
    this.opp.loadBackendTranslation("title", "ozw", true);
  }

  private async _getMigrationData() {
    try {
      this._migrationData = await migrateZwave(this.opp, true);
    } catch (err) {
      showAlertDialog(this, {
        title: "Failed to get migration data!",
        text:
          err.code === "unknown_command"
            ? "Restart Open Peer Power and try again."
            : err.message,
      });
      return;
    }
      ...
      (devices) => {
        ...
        devices.forEach((device) => {
          if (migrationDevices.includes(device.id)) {
            deviceNameLookup[device.id] = computeDeviceName(device, this.opp);
          }
        });
  ...
  private async _doMigrate() {
    const data = await migrateZwave(this.opp, false);
    if (!data.migrated) {
