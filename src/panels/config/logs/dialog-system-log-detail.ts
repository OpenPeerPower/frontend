...
class DialogSystemLogDetail extends LitElement {
        ...
        <div class="contents">
          <p>
            ...
            ${item.count > 0
              ? html`
                  First occurred:
                  ${formatSystemLogTime(item.first_occurred, this.opp!.locale)}
                  (${item.count} occurrences) <br />
                `
              : ""}
            Last logged:
            ${formatSystemLogTime(item.timestamp, this.opp!.locale)}
          </p>
  ...
  private async _fetchManifest(integration: string) {
    try {
      this._manifest = await fetchIntegrationManifest(this.opp, integration);
    } catch (err) {
