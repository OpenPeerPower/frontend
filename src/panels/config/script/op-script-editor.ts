...
export class HaScriptEditor extends KeyboardShortcutMixin(LitElement) {
  ...
  private async _runScript(ev) {
    ev.stopPropagation();
    await triggerScript(this.opp, this.scriptEntityId);
    showToast(this, {
  ...
  private _aliasChanged(ev: CustomEvent) {
    ...
    let id = aliasSlugify;
    let i = 2;
    while (this.opp.states[`script.${id}`]) {
      id = `${aliasSlugify}_${i}`;
      i++;
    }
    this._entityId = id;
  ...
  private _idChanged(ev: CustomEvent) {
    ...
    if (this.opp.states[`script.${this._entityId}`]) {
      this._idError = true;
    } else {
      this._idError = false;
    }
  ...
  private async _delete() {
    await deleteScript(this.opp, computeObjectId(this.scriptEntityId));
    history.back();
  }
  ...
  private _saveScript(): void {
    if (this._idError) {
      showToast(this, {
        ...
        action: {
          action: () => {},
          text: this.opp.localize("ui.dialogs.generic.ok"),
        },
      });
      return;
    }
