...
class HaLogbook extends LitElement {
  ...
  protected shouldUpdate(changedProps: PropertyValues<this>) {
    ...
    const languageChanged =
      oldOpp === undefined || oldOpp.locale !== this.opp.locale;
  ...
  protected updated(_changedProps: PropertyValues) {
    const oldOpp = _changedProps.get("opp") as OpenPeerPower | undefined;

    if (oldOpp === undefined || oldOpp.language !== this.opp.language) {
  ...
  private _renderLogbookItem(
    ...
    const stateObj = item.entity_id
      ? this.opp.states[item.entity_id]
      : undefined;
    const item_username =
    ...
    return html`
      <div class="entry-container">
        ...
        (item?.when &&
          ...
          ? html`
              <h4 class="date">
                ${formatDate(new Date(item.when), this.opp.locale)}
              </h4>
            `
          : html``}
            ...
            <div class="message-relative_time">
              <div class="message">
                ...
                ${item_username
                  ...
                  ? // Service Call
                    ` ${this.opp.localize("ui.components.logbook.by_service")}
                  ${item.context_domain}.${item.context_service}`
                  : item.context_entity_id === item.entity_id
                  ? // HomeKit or something that self references
                    ` ${this.opp.localize("ui.components.logbook.by")}
                  ${
                    item.context_name
                      ? item.context_name
                      : item.context_event_type
                  ...
                  : // Another entity such as an automation or script
                    html` ${this.opp.localize("ui.components.logbook.by")}
                      <a
                        href="#"
                        @click=${this._entityClicked}
