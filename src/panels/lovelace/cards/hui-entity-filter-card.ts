...
class EntityFilterCard extends UpdatingElement implements LovelaceCard {
  public static getStubConfig(
    ...
    return {
      ...
      state_filter: [
        foundEntities[0] ? opp.states[foundEntities[0]].state : "",
      ],
      card: { type: "entities" },
    };
  }
  ...
  protected shouldUpdate(changedProps: PropertyValues): boolean {
    ...
    if (changedProps.has("opp")) {
      return this._haveEntitiesChanged(
        changedProps.get("opp") as OpenPeerPower | null
      );
    }
    return false;
  }
  ...
  private _haveEntitiesChanged(oldOpp: OpenPeerPower | null): boolean {
    if (!this.opp || !oldOpp) {
      return true;
    }
    ...
    if (this.opp.localize !== oldOpp.localize) {
      return true;
    }
