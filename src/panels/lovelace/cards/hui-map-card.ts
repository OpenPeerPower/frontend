...
class HuiMapCard extends LitElement implements LovelaceCard {
  ...
  protected shouldUpdate(changedProps) {
    ...
    if (oldOpp.themes.darkMode !== this.opp.themes.darkMode) {
      return true;
    }
  ...
  protected updated(changedProps: PropertyValues): void {
    if (changedProps.has("opp") || changedProps.has("_history")) {
      this._drawEntities();
      this._fitMap();
    }
    if (changedProps.has("opp")) {
      const oldOpp = changedProps.get("opp") as OpenPeerPower | undefined;
      if (oldOpp && oldOpp.themes.darkMode !== this.opp.themes.darkMode) {
  ...
  private async loadMap(): Promise<void> {
    [this._leafletMap, this.Leaflet, this._tileLayer] = await setupLeafletMap(
      this._mapEl,
      this._config!.dark_mode ?? this.opp.themes.darkMode
    );
    this._drawEntities();
    this._leafletMap.invalidateSize();
    this._fitMap();
  ...
  private _cleanupHistory() {
    ...
    if (this._config!.hours_to_show! <= 0) {
      ...
      this._history = this._history!.reduce(
        (accumulator: OppEntity[][], entityStates) => {
          const entityId = entityStates[0].entity_id;
