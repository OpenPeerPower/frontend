...
export class HuiMediaControlCard extends LitElement implements LovelaceCard {
  ...
  protected render(): TemplateResult {
    ...
    const imageStyle = {
      "background-image": this._image
        ? `url(${this.opp.oppUrl(this._image)})`
        : "none",
  ...
  protected updated(changedProps: PropertyValues): void {
    ...
    if (
      !this._config ||
      !this.opp ||
      (!changedProps.has("_config") && !changedProps.has("opp"))
    ) {
      return;
    }
    ...
    const oldImage =
      oldOpp?.states[this._config.entity]?.attributes.entity_picture_local ||
  ...
  private get _showProgressBar() {
    if (!this.opp || !this._config || this._narrow) {
      return false;
    }
  ...
  private _handleClick(e: MouseEvent): void {
    ...
    this.opp!.callService("media_player", action, {
      entity_id: this._config!.entity,
    });
  }
  ...
  private get _stateObj(): MediaPlayerEntity | undefined {
    return this.opp!.states[this._config!.entity] as MediaPlayerEntity;
  }
