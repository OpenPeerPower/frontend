...
export const generateLovelaceConfigFromData = async (
  ...
  if (opp.config.safe_mode) {
    return {
      title: opp.config.location_name,
      views: [
        {
  ...
  if (
    ...
    if (isComponentLoaded(opp, "geo_location")) {
...
export const generateLovelaceConfigFromOpp = async (
  opp: OpenPeerPower,
  localize?: LocalizeFunc
): Promise<LovelaceConfig> => {
  if (opp.config.state === STATE_NOT_RUNNING) {
    return {
      title: opp.config.location_name,
      views: [
        {
