...
export class HuiCardPicker extends LitElement {
  ...
  protected shouldUpdate(changedProps: PropertyValues): boolean {
    ...
    if (oldOpp.locale !== this.opp!.locale) {
      return true;
    }
  ...
  protected firstUpdated(): void {
    ...
    const unusedEntities = calcUnusedEntities(this.opp, usedEntities);

    this._usedEntities = [...usedEntities].filter(
      (eid) =>
        this.opp!.states[eid] &&
        !UNAVAILABLE_STATES.includes(this.opp!.states[eid].state)
    );
      ...
      element: html`${until(
        ...
        html`
          <div class="card spinner">
            <op-circular-progress active alt="Loading"></op-circular-progress>
          </div>
        `
      )}`,
    }));
