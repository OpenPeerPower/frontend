...
export abstract class HuiElementEditor<T> extends LitElement {
    ...
    return html`
      <div class="wrapper">
        ...
        ${this.hasError
          ? html`
              <div class="error">
                ${this.opp.localize("ui.errors.config.error_detected")}:
                <br />
                <ul>
        ...
        ${this.hasWarning
          ? html`
              <div class="warning">
                ${this.opp.localize("ui.errors.config.editor_not_supported")}:
                <br />
                ${this._warnings!.length > 0 && this._warnings![0] !== undefined
                  ...
                  : ""}
                ${this.opp.localize("ui.errors.config.edit_in_yaml_supported")}
              </div>
            `
          : ""}
      </div>
  ...
  protected updated(changedProperties: PropertyValues) {
    ...
    if (this._configElement && changedProperties.has("opp")) {
    ...
    try {
      ...
      if (this._configElementType !== this.configElementType) {
        ...
        if (!this.configElementType) {
          throw new Error(
            this.opp.localize("ui.errors.config.no_type_provided")
          );
        }
      ...
      if (this._configElement) {
        ...
        try {
          ...
          const msgs = handleStructError(this.opp, err);
