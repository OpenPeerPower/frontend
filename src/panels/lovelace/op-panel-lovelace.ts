...
class LovelacePanel extends LitElement {
  ...
  public connectedCallback(): void {
    ...
    if (
      this.lovelace &&
      this.opp &&
      this.lovelace.locale !== this.opp.locale
    ) {
  ...
  protected render(): TemplateResult | void {
    ...
    if (state === "error") {
      return html`
        <opp-error-screen
          .opp=${this.opp}
          title="${domainToName(this.opp!.localize, "lovelace")}"
  ...
  private _lovelaceChanged() {
    ...
    showToast(this, {
      ...
      action: {
        ...
        text: this.opp!.localize("ui.common.refresh"),
      },
      duration: 0,
      dismissable: false,
    });
  ...
  private async _fetchConfig(forceDiskRefresh: boolean) {
    ...
    if (!resourcesLoaded) {
      ...
      (
        llWindow.llConfProm || fetchResources(this.opp!.connection)
      ).then((resources) =>
    ...
    try {
      ...
      const localize = await this.opp!.loadBackendTranslation("title");
      conf = await generateLovelaceConfigFromOpp(this.opp!, localize);
      confMode = "generated";
    } finally {
    ...
    this.lovelace = {
      ...
      saveConfig: async (newConfig: LovelaceConfig): Promise<void> => {
        ...
        try {
          ...
          this._ignoreNextUpdateEvent = true;
          await saveConfig(this.opp!, urlPath, newConfig);
        } catch (err) {
          // eslint-disable-next-line
          console.error(err);
      ...
      deleteConfig: async (): Promise<void> => {
        ...
        try {
          ...
          const localize = await this.opp!.loadBackendTranslation("title");
          this._updateLovelace({
            config: await generateLovelaceConfigFromOpp(this.opp!, localize),
            mode: "generated",
            editMode: false,
          });
          this._ignoreNextUpdateEvent = true;
          await deleteConfig(this.opp!, urlPath);
        } catch (err) {
          // eslint-disable-next-line
          console.error(err);
          // Rollback the optimistic update
