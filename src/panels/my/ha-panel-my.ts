...
class HaPanelMy extends LitElement {
  ...
  connectedCallback() {
    ...
    if (path.startsWith("supervisor")) {
      ...
      navigate(
        this,
        `/oppio/_my_redirect/${path}${window.location.search}`,
        true
      );
      return;
    }
    ...
    if (
      redirect.component &&
      !isComponentLoaded(this.opp, redirect.component)
    ) {
          ...
          error =
            this.opp.localize(
              ...
              html`<a
                ...
                >${this.opp.localize("ui.panel.my.faq_link")}</a
              >`
        ...
        case "no_supervisor":
          error = this.opp.localize(
            ...
            html`<a
              target="_blank"
              rel="noreferrer noopener"
              href="${documentationUrl(this.opp, "/installation")}"
              >${this.opp.localize("ui.panel.my.documentation")}</a
            >`
          );
          break;
        default:
          error = this.opp.localize("ui.panel.my.error") || "Unknown error";
      }
      return html`<opp-error-screen .error=${error}></opp-error-screen>`;
    }
    return html``;
  }
